...
switch method
    case 0
       t_thr =  prctile(X(~ix), 100-fc);
       b_thr =  prctile(X(~ix), fc);
    case 1   
       t_thr =  mean(X(~ix)) + std(X(~ix))*fc;
       b_thr =  mean(X(~ix)) - std(X(~ix))*fc;
    case 2   
       [~,thr]=boundary(paretotails(X(:),(100-fc)/100,fc/100));
       t_thr = thr(2);
       b_thr = thr(1);
end
Xw(~ix) = max(min(X(~ix),t_thr),b_thr);    
end
%------------- END OF CODE --------------
