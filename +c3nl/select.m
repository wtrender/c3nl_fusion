...
%% C3NL.SELECT: Wrapper to bash find 
%
%    __           _             
%   / _|         (_)            
...
switch sel.out
    ...
    case 'name'
        S = strrep(S,[obj.pth,filesep],'');
        S = convert.string2cell(S,'\n');
        S(c3nl.strDetect(S,['.|',obj.pth]))=[];
    otherwise
...
for p=prop
    ...
    switch p{1}
        case 'maxdepth';if ia;mxd=sprintf(' -maxdepth %i', input{ia+1});else; mxd='';end
        case 'mindepth';if ia;mnd=sprintf(' -mindepth %i', input{ia+1});else; mnd='';end
        case 'name';if ia;nam=[' -name "' input{ia+1} '"'];else; nam='';end
        case 'type';if ia;typ=[' -type ' input{ia+1}];else; typ='';end    
        case 'exec';if ia;exc=[' -exec ' input{ia+1}];else; exc='';end
        case 'pth';if ia;pth=input{ia+1};else; pth=' $PWD ';end 
        case 'delete';if ia;del=' -delete ';else; del='';end        
        case 'output';if ia;out=input{ia+1};else; out='cell';end                           
        case 'regex';if ia;reg=[' -regex ' input{ia+1}];else; reg='';end
        case 'printf';if ia;prnf=[' -printf ' input{ia+1}];else; prnf='';end 
        case 'sel'
