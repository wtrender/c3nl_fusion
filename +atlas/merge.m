...
if isempty(sid)
    ...
    pd = size(Pmv);
    nd = numel(pd);
    %stats = regionprops3(Pmv,{'Volume','Centroid'});
    
%     plot.imOverlay(get.montage(Pmv,3),get.montage(dz,3),.8);
    d = c3nl.neighb(pd,1,'euc');
    V = Pmv;
    % find all overlapping voxels 
    for ii=1:numel(sid)
        pidx =find(ismember(Pmv,sid(ii)));
        %plot.imOverlay(get.montage(Pmv,3),get.montage(ismember(Pmv,sid(ii)),3),.3);
        jj=numel(pidx);
        while ~isempty(pidx)
            ix = pidx(end)+d;
            c=Pmv(unique(ix));
            c = c(ismember(c,cid));
            %tix = ismember(id,unique(tmp));
            %[~,tix] = min(dist.euc(stats.Centroid(id==sid(ii),:),stats.Centroid(tix,:)));
            if ~isempty(c)
                V(pidx(end)) = mode(c);
                pidx(end)=[];
            else
