function [y_red_M, expanded0] = teg_RMA_inner_recode_raw(M_raw, NM, cellsets, iPred)

% Recodes raw data matrix M into matrix with values for each combination of
% levels of factors involved in the current test.
%
% Output:
%
% y_red_M has one column per combination of levels.
%
% expanded0 has the same size of the original M, with the same values in
% columns belonging to the same combination of levels.
