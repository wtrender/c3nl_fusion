function [X1, factorStarts1, nColsfactor1, labels, cellsets, factors] = teg_RMA_create_ANOVA_dummy(varargin)

% Create matrix of dummy predictors for testing all combinations of
% factors.
...
id_matrix = teg_rec_combinations(levels, 0);
...
for tuple = 2:length(levels),
    ...
    fac_comb = teg_rec_combinations(tmp0, 1);
    for iFacComb = 1:size(fac_comb, 1),
        factorStarts{tuple}(iFacComb) = fs0;
        facs0 = fac_comb(iFacComb, :);
        levels_matrix = teg_rec_combinations(levels(facs0) - 1, 0);
        newsubmatrix = [];
        ...
        levels_matrix = teg_rec_combinations(levels(facs0), 0);
        for iLevelComb = 1:size(levels_matrix, 1),
            fcs = ones(1, nComb);
            for iFactor = 1:size(levels_matrix, 2),
