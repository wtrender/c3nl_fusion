function output = boost(X,Y,S,seed,ho,k)
...
rng(seed);
ix = get.partition(S,'holdout',ho,seed);
XR = X(~ix,:);YR = Y(~ix);
Xt = X(ix,:);Yt = Y(ix);
t = templateTree('MaxNumSplits',5);
CV = fitcensemble(Xt,Yt,'Method','RobustBoost','Learners',t,'CrossVal','on','NumLearningCycles',10);
Null = fitcensemble(Xt,Yt(randperm(size(Yt,1))),'Method','RobustBoost','Learners',t,'NumLearningCycles',10); % estimate Null
EC = fitcensemble(Xt,Yt,'Method','AdaBoostM1','Learners',t,'NumLearningCycles',10);
