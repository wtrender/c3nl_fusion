function [measuresOrder,featuresOrder,featureClusters,measuresClusters,fhsv]=plotHCLvert(X,obj,dmethod,lmethod,cmap,mmap)
...
Y = inconsistent(Z_features);
[~,id]=sort(Y(:,4),'descend');
%[~,id]=max(Y(:,2))
featuresOrder = optimalleaforder(Z_features,pdist(Data));
axes(ax.dendoleft);[h,~,Z_features_order,lcf] = dendrogram(Z_features,0,'orient', 'left', 'colorthreshold',Z_features(id(1),3),'Reorder',featuresOrder);
set(h, 'linewidth', 1);axis(ax.dendoleft,'off')
conn = (Z_features(:,3) < Z_features(id(1),3));
[featureClusters,L] = mylabeltree(Z_features, conn,lcf);
Y = inconsistent(Z_measures,5);
[~,id]=sort(Y(:,4),'descend');
...
if ~exist('cmap','var')
    fhsv = helper.diffcolor([45,355],[0.5,.85],[0.45,1],max(featureClusters));
else 
    fhsv = cmap;
end
...
axes(ax.dendotop);[h,~,Z_measures_order,lc]=dendrogram(Z_measures,0,'Reorder',measuresOrder, 'colorthreshold',Z_measures(id(1),3));
conn = (Z_measures(:,3) < Z_measures(id(1),3));

[measuresClusters,L] = mylabeltree(Z_measures, conn,lc);
% if ~exist('mmap','var')
%     mhsv = helper.diffcolor([0,150],[0.5,0.8],[0.6,0.7],max(featureClusters));
% end
% %lcf = branch2node(lcf,measuresClusters);
% for ii=1:numel(lc)
%     if lc(ii)
%         h(ii).Color =  mhsv(lc(ii),:);
%         h(ii).LineWidth = 2;
%     end
% end
%axes(ax.dendoleft);[h,~,Z_features_order] =
%dendrogram(Z_features,0,'orient', 'left'); , 'colorthreshold', Z_measures(id(2),3)
%axes(ax.dendotop);if srt;[h,~,Z_measures_order]=dendrogram(Z_measures,0);else [~,Z_measures_order]=sort(Y2);end
%set(h, 'Color', 'k');
...
gc = gen.cmap([120 34 102;230 210 230]);
...
if ~isempty(cat)
    GC = gen.ind2Cmap(gc,double(cat));
    if numel(cat)<4
        [x,y] = gen.fancyGrid(numel(cat),0.1,0.1,'same');
        y(:) = 0.5;
    else
        r = round(numel(cat)^.5);
        A = ones(r,1)*(numel(cat)/r);
        [x,y] = gen.fancyGrid(A,0.1,0.1,'same');
    end
    x=flipud(x);
    for jj=1:numel(cat)
...
function [lc,featureClusters] = branch2node(lc,featureClusters)
    % nodes
...
for ii=1:length(ia)
    ...
    switch tmp
        ...
        case 'c0';if ia(ii);c0=obj.(tmp);else c0=gen.cmap(flipud([255 205 155;255 180 104;255 155 54;255 130 4;191 98 3;51 51 51;14 83 164;49 92 143;62 117 182;110 152 200;207 221 237]./255),101);end
        ...
        case 'ax'
            if ia(ii);ax=obj.(tmp);
                ...
                if any(gp)
                    A = ones(sum(gp),1);
                    [x,y,w,h]=gen.fancyGrid(A,0.05,0.001,'same');
